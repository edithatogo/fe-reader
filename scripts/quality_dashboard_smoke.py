...
SCHEMA = ROOT / "schemas/public-quality-dashboard.schema.json"
CRATE = ROOT / "crates/fe_reader_quality_dashboard/src/lib.rs"
...
def main() -> None:
    ...
    if not SCHEMA.exists():
        fail(f"missing schema evidence file: {SCHEMA.relative_to(ROOT)}")
    if not CRATE.exists():
        fail(f"missing crate evidence file: {CRATE.relative_to(ROOT)}")
    for token in [
