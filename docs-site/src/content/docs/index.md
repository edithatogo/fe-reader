...
- Adapter crates for rendering, PDF parsing, automation, plugins, bindings, and platform integration. PDF capability claims are governed by `docs/pdf-parity-registry.md`.
...
Fe Reader is not a clone of any single PDF vendor. It is a privacy-preserving, evidence-oriented PDF platform that aims to meet baseline PDF expectations while exposing the verification and safety machinery that professional workflows need. PDF capability claims are governed by `docs/pdf-parity-registry.md`.
